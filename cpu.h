...
typedef struct {
	...
	unsigned long FPCR[32];
	REGISTER HI;
	REGISTER LO;
	unsigned long RegRDRAM[10];
	unsigned long RegSP[10];
	unsigned long RegDP[10];
	unsigned long RegMI[4];
	unsigned long RegVI[14];
	unsigned long RegAI[6];
	unsigned long RegPI[13];
	unsigned long RegRI[8];
	unsigned long RegSI[4];
	TLB tlb[32];
	char PifRAM[0x40];
} SaveState;
...
// these definitions swiped from Project64
...
extern unsigned long FAKE_CAUSE_REGISTER;
...
#define DPC_START_REG			state->RegDP[0]
#define DPC_END_REG				state->RegDP[1]
#define DPC_CURRENT_REG			state->RegDP[2]
#define DPC_STATUS_REG			state->RegDP[3]
#define DPC_CLOCK_REG			state->RegDP[4]
#define DPC_BUFBUSY_REG			state->RegDP[5]
#define DPC_PIPEBUSY_REG		state->RegDP[6]
#define DPC_TMEM_REG			state->RegDP[7]
...
#define PROGRAM_COUNTER			(state->PC)
...
extern long JumpToLocation, NextInstruction, LLBit, LLAddr, WaitMode, AudioIntrReg, ViFieldNumber, HalfLine;
