...
int CPUHLE_Scan(void)
{
	...
	for(i=0; i < (state->RamSize - 64); i+=4) {

		for(entry = 0; entry < numEntries; entry++) {

			if(entrys[entry].used)
				continue;

			good = 1;
			entrySize = entrys[entry].length;

			for(j=0; j < entrySize; j++) {
				if(RAM_Pages[(i+j+0x75c) >> 16]==0) {
					good = 0;
					break;
				}

				address = PageRAM2(i+j);
				//address = i;
				c = *(unsigned char *)(address);
				d = entrys[entry].bytes[j^3];
			...
			if(good == 1 && i< (state->RamSize-64)) {
				char buf[256];
				if(needcomma) {
					sprintf(buf,", %s", entrys[entry].name);
				} else {
					sprintf(buf,"%s", entrys[entry].name);
				}
...
{
	int i = 0;
	unsigned long real_addr = PageVRAM3(loc);

	for(i = 0; i < numEntries; i++) {

		if(entrys[i].used && (entrys[i].phys == real_addr)) {
// 			printf("CPU HLEing using %d at %08x (phys: %08x) \"%s\"\n", entrys[i].name, loc, entrys[i].phys, entrys[i].name);

			if(entrys[i].location(entrys[i].phys))
			{
// 				printf("done\n");
				return 1;
			}
