...
void FreeSection (BLOCK_SECTION * Section, BLOCK_SECTION * Parent)
{
    ...
    if (Section->ParentSection)
    {
        ...
        if (Section->ParentSection)
        {
            for (count = 0; count < NoOfParents; count++)
            {
                if (!IsAllParentLoops(Section,Section->ParentSection[count],0,GetNewTestValue()))
                {
                    return;
                }
            }
...
void GenerateSectionLinkage (BLOCK_SECTION * Section)
{
    ...
    {
        if (Section->Cont.LinkLocation == NULL && Section->Cont.FallThrough == 0)
        {
            Section->ContinueSection = NULL;
        }
        if (Section->Jump.LinkLocation == NULL && Section->Jump.FallThrough == 0)
        {
            Section->JumpSection = NULL;
        }
...
#ifdef USEX64
// orig    MoveConstQwordToX86reg(TLB_Map, x86_R15);
    MoveConstQwordToX86reg((uintptr_t)TLB_Map, x86_R15);
#endif
    ...
    do
    {
        ...
        case R4300i_REGIMM:
            ...
            {
            case R4300i_REGIMM_BLTZ:
                Compile_R4300i_Branch(Section,BLTZ_Compare,BranchTypeRs, 0);
                break;
            case R4300i_REGIMM_BGEZ:
                Compile_R4300i_Branch(Section,BGEZ_Compare,BranchTypeRs, 0);
                break;
            case R4300i_REGIMM_BLTZL:
            ...
            case R4300i_REGIMM_BGEZL:
                ...
                break;
            case R4300i_REGIMM_BLTZAL:
                Compile_R4300i_Branch(Section,BLTZ_Compare,BranchTypeRs, 1);
                break;
            case R4300i_REGIMM_BGEZAL:
                Compile_R4300i_Branch(Section,BGEZ_Compare,BranchTypeRs, 1);
                break;
            default:
                Compile_R4300i_UnknownOpcode(Section);
                break;
        ...
        case R4300i_CP1:
            ...
            case R4300i_COP1_BC:
                ...
                {
                case R4300i_COP1_BC_BCF:
                    Compile_R4300i_Branch(Section,COP1_BCF_Compare,BranchTypeCop1,0);
                    break;
                case R4300i_COP1_BC_BCT:
                    Compile_R4300i_Branch(Section,COP1_BCT_Compare,BranchTypeCop1,0);
                    break;
                case R4300i_COP1_BC_BCFL:
                    Compile_R4300i_BranchLikely(Section,COP1_BCF_Compare,0);
                    break;
                case R4300i_COP1_BC_BCTL:
                    Compile_R4300i_BranchLikely(Section,COP1_BCT_Compare,0);
                    break;
                default:
                    Compile_R4300i_UnknownOpcode(Section);
                    break;
...
uint32_t IsAllParentLoops(BLOCK_SECTION * Section, BLOCK_SECTION * Parent, uint32_t IgnoreIfCompiled, uint32_t Test)
{
    ...
    {
        if (!IsAllParentLoops(Section,Parent->ParentSection[count],IgnoreIfCompiled,Test))
        {
            return 0;
        }
    }
