...
void CompileReadTLBMiss (BLOCK_SECTION * Section, int32_t AddressReg, int32_t LookUpReg )
{
    ...
    CompileExit(Section->CompilePC,&Section->RegWorking,TLBReadMiss,0,(void*)&JeLabel32);
}
...
void Compile_R4300i_Branch (BLOCK_SECTION * Section, void (*CompareFunc)(BLOCK_SECTION * Section), int32_t BranchType, uint32_t Link)
{
    ...
    if ( NextInstruction == NORMAL )
    {
        ...
        if ((Section->CompilePC & 0xFFC) != 0xFFC)
        {
            ...
            {
            case BranchTypeRs:
                EffectDelaySlot = DelaySlotEffectsCompare(Section->CompilePC,Opcode.rs,0);
                break;
            case BranchTypeRsRt:
                EffectDelaySlot = DelaySlotEffectsCompare(Section->CompilePC,Opcode.rs,Opcode.rt);
                break;
            case BranchTypeCop1:
            {
                OPCODE Command;
